...
## GGPLOT
First load the package into R using either `library()` or `require()`
```{r}

```


And be sure to load in the Tesla Data:
```{r}
tesla<-read.csv()
```

**Your Turn 1** Talk in a group - what relationship do you expect to see in the `tesla` data between the age of the battery (`Battery AgeDays`) and the amount of mileage put on the car since the battery was installed (`MileageSinceNewKM`)?  (No peeking!) Type in your answer here.
...
Did the plot match your predictions? What do you notice?





Let's add on some more layers! What if we want to make the variable Replacement Battery to a color aesthetic?  Run the following code and note what happens.
...
Now run the following code which places "color" outside of the `aes()` code.  What happens?   
```{r}
ggplot(data=tesla) + 
  geom_point(mapping=aes(x=BatteryAgeDays, y=MileageSinceNewKM), color="blue")
```

What is the different between placing an argument within the `aes()` code versus outside of it?
...
There are a lot of resources available, including a the ["ggplot2 Cheatsheet"](Documents/ggplot2 Cheatsheet.pdf).  Open up the cheatsheet and look through it.

**Your Turn 2**  Make the shown density plot, using the Cheatsheet to help you.  What does the plot tell you about the Tesla driver habits in different regions?
...
**If Time** Try a few more types of graphs and explore the tesla battery data on your own.  Try to recreate some of the plots you made in base R using `ggplot2`.